...
def start(model_class, quantize_method, LR_FACTOR, total_bits, with_activation, EPOCHS, starting_checkpoint, KEEP_FACTOR, batch_size=32, adder=None, with_bias=True, fused_bnorm=True):
    ...
    quantize_method(model=model, total_bits=total_bits, FLEX_SAMPLE_SIZE=FLEX_SAMPLE_SIZE, with_activation=with_activation, KEEP_FACTOR=KEEP_FACTOR, with_bias=with_bias, adder=adder, debug=False)
    ...
    with open(f"{val_path}/rccm_results3.txt", "a") as text_file:
        text_file.write(str(evaluation_results) + "   argv: " + str(sys.argv) + "\n")