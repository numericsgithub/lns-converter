...
import mquat as mq
import mquat.Datasets.CiFar100 as cifar100
import matplotlib.pyplot as plt
import datetime

mq.FlexPointQuantizer.error_statistic = tf.Variable([], name="error_statistic", shape=[None], trainable=False, dtype=tf.float32, validate_shape=False)

def start(model_class, quantize_method, LR_FACTOR, total_bits, KEEP_FACTOR, with_activation, EPOCHS, starting_checkpoint, batch_size=1024):
    ...
    train_dataset = cifar100.get_train_ds(TRAIN_BATCH_SIZE, train_record_path)
    test_dataset = cifar100.get_test_ds(TEST_BATCH_SIZE, test_record_path)
    ...
    model = model_class([cifar100.IMAGE_CROP_SIZE, cifar100.IMAGE_CROP_SIZE, 3], [100], [100], WEIGHT_DECAY, tainable_bias=True, do_batch_norm=False)
    ...
    model.build([None, cifar100.IMAGE_CROP_SIZE, cifar100.IMAGE_CROP_SIZE, 3])
    model.summary()
    ...
    # all_rounding_errors = mq.FlexPointQuantizer.error_statistic.numpy()
    # a = all_rounding_errors
    # num_bins = 1024
    ...
    class LocalCallbacks(callbacks.Callback):
        def __init__(self, best_acc):
            ...
            self.train_log_name = "trian_log_" + model_quant_desc + ".txt"
        ...
        def on_train_end(self, logs={}):
            print("found the best one for", model_quant_desc, " with ", logs["val_top 1 fixed"],
                  logs["val_top 5 fixed"], flush=True)
            f = open(self.train_log_name, "a", buffering=1)
            f.write("found a better one for" + model_quant_desc+ " with "+ str(logs["val_top 1 fixed"])+ str(logs["val_top 5 fixed"]) + "\n")
            f.flush()
            f.close()


    csv_logger = callbacks.CSVLogger('log.csv', append=True, separator=';')
    log_dir = "logs/fit/" + datetime.datetime.now().strftime("%d_%m_%Y-%H_%M_%S")
    ...
    model.fit(train_dataset, epochs=EPOCHS, validation_data=test_dataset, verbose=1, use_multiprocessing=True,
              validation_freq=1, callbacks=[LocalCallbacks(0.0), csv_logger, tensorboard_callback]) # tensorboard_callback

    # model.saveVariablesNPZ(f"{model_quant_desc}_last", quantisize=False)
    # model.saveOptimizerNPZ(f"opt_{model_quant_desc}_last")
    # model.saveVariablesNPZ(f"{model_quant_desc}_quan_last", quantisize=True)
