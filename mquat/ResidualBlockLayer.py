...
class ResidualBlockLayer(ComplexLayer):
    ...
    Parameters:
        ...
        channels_list (list of int):
            number of output channels in each convolutional layer (except shortcut).
        kernel_size_list (list of ints):
            size of the square kernels.
        first_stride (int):
            stride (x and y direction equal) of the first convolution layer in the block.
    ...
    def __init__(self, name, channels_list, kernel_size_list, first_stride, has_conv_shortcut=False,
                 kernel_initializer="he_normal", kernel_regularizer=None,
                 bias_initializer="zeros", bias_regularizer=None, do_batch_norm=True, tainable_bias=False):
        super().__init__(name, tf.identity, False)
        self.conv_1 = Conv2DLayer(name + "_conv_1", channels_list, kernel_sizes=(3, 3), strides=first_stride, padding="VALID",
                                  activation_func=tf.identity, do_batch_norm=do_batch_norm, tainable_bias=tainable_bias)
        self.conv_2 = Conv2DLayer(name + "_conv_2", channels_list, kernel_sizes=(3, 3), strides=(1, 1), padding="VALID",
                                  activation_func=tf.identity, do_batch_norm=do_batch_norm, tainable_bias=tainable_bias)
        ...
        if has_conv_shortcut:
            self.conv_short = Conv2DLayer(name + "_conv_short", channels_list, kernel_sizes=(1, 1), strides=first_stride, padding="VALID",
                                          activation_func=tf.identity, do_batch_norm=do_batch_norm, tainable_bias=tainable_bias)
        else:
    ...
    def getQuantizers(self):
        ...
        """
        quantizers = []
        quantizers.extend(self.conv_1.getQuantizers())
        quantizers.extend(self.conv_2.getQuantizers())
        if self.conv_short != None:
            quantizers.extend(self.conv_short.getQuantizers())
        return quantizers
    ...
    def isQuantisizing(self):
        ...
        """
        return self.conv_1.isQuantisizing() or self.conv_2.isQuantisizing() or self.input_add.isQuantisizing() or self.quant_out.isQuantisizing()
    ...
    def call(self, inputs, training):
        ...
        """
        tmp = tf.pad(inputs, [[0, 0], [1, 1], [1, 1], [0, 0]], "CONSTANT")
        tmp = self.conv_1(tmp, training)
        tmp = tf.nn.relu(tmp)
        tmp = tf.pad(tmp, [[0, 0], [1, 1], [1, 1], [0, 0]], "CONSTANT")
        tmp = self.conv_2(tmp, training)
        if self.conv_short != None:
            inputs = self.conv_short(inputs, training)
        tmp = self.input_add([tmp, inputs])
        tmp = tf.nn.relu(tmp)
        return self.quant_out(tmp, training)
    ...
    def get_config(self):
        ...
        config["type"] = "mquat.ResidualBlockLayer"
        return config